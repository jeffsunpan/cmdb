...
class ANSRunner(object):   
    ...
    def __initializeData(self):  
        ...
        self.options = Options(connection='smart', module_path=None, forks=100, timeout=10,  
                remote_user='root', ask_pass=False, private_key_file=None, ssh_common_args='-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o CheckHostIP=false', ssh_extra_args=None,  
...
if __name__ == '__main__':
    ...
    resource =  { 
                    "dynamic_host": {  #定义的动态主机名，需要跟playbook里面的hosts对应
                        "hosts": [
                                    {"hostname": "127.0.0.1", "port": "22", "username": "root", "password": "123456"},
                                    #{"hostname": "10.70.75.66", "port": "22", "username": "root"},
    ...
    rbt.run_model(host_list=["127.0.0.1"],module_name='setup',module_args='')
    ...
    #print data
    a=json.loads(data)['success']
 
    for ip in a:
        Total_Memory=a[ip]['ansible_facts']['ansible_memory_mb']['real']['total']   
        Default_IP=a[ip]['ansible_facts']['ansible_default_ipv4']['address']
        CPUs=a[ip]['ansible_facts']['ansible_processor_vcpus']
        Arch=a[ip]['ansible_facts']['ansible_architecture']
        Distribution=a[ip]['ansible_facts']['ansible_distribution']
        Kernel=a[ip]['ansible_facts']['ansible_kernel']
        DistributionVersion=a[ip]['ansible_facts']['ansible_distribution_version']

        print 'hosts[' + ip +']' + Default_IP,Total_Memory,CPUs,Arch,Distribution,Kernel,DistributionVersion
