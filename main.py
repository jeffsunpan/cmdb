#!/usr/bin/env python
#coding=utf-8
#更多内容请参考博客：http://www.linuxyw.comss
#戴儒锋
#63780668@qq.com
import os,sys,json
...
def hosts():
    s = request.environ.get('beaker.session')
    return template('hosts',session=s)

@route('/api/gethostsinfo',method=['GET', 'POST'])
@checkLogin
def gethostsinfo():
    sql = """
        SELECT
...
def infohost(id):
    ...
    host_data = readDb(host_sql,(id))
    return template('infohost',item_data=host_data)
...
def addhost():
    ...
    """
    item_data = readDb(item_sql,)
    return template('addhost',info={},item_data=item_data)
...
def addhost():
    ...
    sql = "INSERT INTO hosts(ip,port,project,remark) VALUES(%s,%s,%s,%s)"
    data = (ip,port,name,remark)
    result = writeDb(sql,data)
    ...
    data = (DistributionVersion, Kernel, Total_Memory, Distribution, Arch, CPUs, ip)
...
def delhosts():
    ...
    sql = "delete from hosts where id in (%s)"
    result = writeDb(sql,id)
    if result:
        return '0'
    else:
...
@route('/updatehostsinfo')
@checkLogin
def updatehostsinfo():

    ip_address = []
    sql_ip = "select ip from hosts;"
    result_ip = readDb(sql_ip,)
