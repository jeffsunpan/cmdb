
<!-- Page Sidebar -->
<div class="page-sidebar" id="sidebar">
    <!-- Page Sidebar Header-->
    <div class="sidebar-header-wrapper">
        <input type="text" class="searchinput" />
        <i class="searchicon fa fa-search"></i>
        <div class="searchhelper">搜索</div>
    </div>
    <!-- /Page Sidebar Header -->

    <!-- Sidebar Menu -->
    <ul class="nav sidebar-menu">
        <!--Dashboard-->
        <li>
            <a href="/">
                <i class="menu-icon glyphicon glyphicon-home"></i>
                <span class="menu-text"> 首页 </span>
            </a>
        </li>
		<li class="active open">
            <a href="#" class="menu-dropdown">
                <i class="menu-icon fa fa-gavel"></i>
                <span class="menu-text"> 服务器信息 </span>
                <i class="menu-expand"></i>
            </a>
            <ul class="submenu">
                <li class="">
                    <a href="/hosts">
                        <span class="menu-text">主机信息</span>
                    </a>
                </li>
                <li class="">
                    <a href="/updatehostsinfo">
                        <span class="menu-text">更新主机</span>
                    </a>
                </li>
            </ul>
        </li>

		<li class="active open">
            <a href="#" class="menu-dropdown">
                <i class="menu-icon fa fa-gavel"></i>
                <span class="menu-text"> 项目管理 </span>
                <i class="menu-expand"></i>
            </a>
            <ul class="submenu">
                <li class="">
                    <a href="/item">
                        <span class="menu-text">项目列表</span>
                    </a>
                </li>
				<li class="">
                    <a href="/additem">
                        <span class="menu-text">发布项目</span>
                    </a>
                </li>
                <li>
                    <a href="/dropitem">
                        <span class="menu-text">项目回收站</span>
                    </a>
                </li>

            </ul>
        </li>

        <li class="active open">
            <a href="#" class="menu-dropdown">
                <i class="menu-icon fa fa-gavel"></i>
                <span class="menu-text"> 用户管理 </span>
                <i class="menu-expand"></i>
            </a>
            <ul class="submenu">
                <li class="">
                    <a href="/user">
                        <span class="menu-text">用户管理</span>
                    </a>
                </li>
                <li class="">
                    <a href="/department">
                        <span class="menu-text">部门管理</span>
                    </a>
                </li>

            </ul>
        </li>

        <li class="active open">
            <a href="#" class="menu-dropdown">
                <i class="menu-icon fa fa-bar-chart-o"></i>
                <span class="menu-text"> 任务管理 </span>
                <i class="menu-expand"></i>
            </a>
            <ul class="submenu">
                <li class="">
                    <a href="/taskinfo">
                        <span class="menu-text">任务大厅</span>
                    </a>
                </li>
                <li class="">
                    <a href="/addtask">
                        <span class="menu-text">发布任务</span>
                    </a>
                </li>
                <li class="">
                    <a href="/mytask">
                        <span class="menu-text">我的任务</span>
                    </a>
                </li>
                <li>
                    <a href="/mysendtask">
                        <span class="menu-text">我发起的任务</span>
                    </a>
                </li>
                <li>
                    <a href="/droptask">
                        <span class="menu-text">任务回收站</span>
                    </a>
                </li>

            </ul>
        </li>


    </ul>
    <!-- /Sidebar Menu -->
</div>
<!-- /Page Sidebar -->
<!-- Page Content -->
